...
// Ensure the FCP label is on any PRs that are in FCP.
// If it's there, no need to do anything.
async function ensureFCPLabel(labels, num) {
    // If FCP label doesn't exist, add it by calling the API.
    if (!labels.includes(fcpLabel)) {
        await github.issues.addLabels({ ...baseParameters, issue_number: num, labels: [fcpLabel] });
    } else { };
}
    ...
    for (const pull of pulls) {
        ...
        for (comment of prComments) {
            ...
            if (comment.body.includes("start-fcp")) {
                ...
                // Ensure we have the FCP label.
                await ensureFCPLabel(labels, pull.number);
                // Check the FCP time.
