...
public abstract class JavadocLintTask extends DefaultMappingsTask {
    public static final String TASK_NAME = "javadocLint";
    ...
    public JavadocLintTask() {
        super(Constants.Groups.JAVADOC_GENERATION);
    ...
    public abstract static class LintAction implements WorkAction<LintParameters> {
        ...
        public void execute() {
            try {
                var files = getParameters().getMappingFiles().getFiles().stream().map(File::toPath).toArray(Path[]::new);
